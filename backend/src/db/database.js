...
async function createPostgreSQLTables() {
  ...
  try {
    ...
    try {
      ...
      if (!indexError.message.includes('already exists') && !indexError.code?.includes('42P07')) {
        console.warn('Warning creating processing_history index:', indexError.message);
      }
    }
  } catch (tableError) {
    // Table might already exist, but log other errors
    if (!tableError.message.includes('already exists') && !tableError.code?.includes('42P01')) {
  ...
  await client.query(`
    CREATE TABLE IF NOT EXISTS datasets (
      ...
      "pricePerRecordHBAR" DECIMAL(18, 8),
      "pricePerRecordUSD" DECIMAL(18, 8),
      "pricingCategoryId" VARCHAR(32),
      "pricingCategory" VARCHAR(100),
...
async function createSQLiteTables() {
  ...
  await run(`
    CREATE TABLE IF NOT EXISTS datasets (
      ...
      price REAL NOT NULL,
      "priceUsd" REAL,
      "pricePerRecordHBAR" REAL,
      "pricePerRecordUSD" REAL,
      "pricingCategoryId" TEXT,
      "pricingCategory" TEXT,
      "volumeDiscount" REAL DEFAULT 0,
